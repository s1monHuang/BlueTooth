...
- (void)viewDidLoad {
    ...
    _everydayTrainLabel.text = BTLocalizedString(@"每日运动目标");
    _otherLabel.text = BTLocalizedString(@"根据您的个人情况设置合理目标");
...
//        _btnNext.alpha = 1;
        ...
        [btnNext addTarget:self action:@selector(changeTrainTarget) forControlEvents:UIControlEventTouchUpInside];
        [btnNext setTitle:BTLocalizedString(@"完成") forState:UIControlStateNormal];
    ...
    if (first == 1) {
        ...
        self.operateVM.finishHandler = ^(BOOL finished, id userInfo) { // 网络数据回调
            if (finished) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:userInfo message:nil delegate:nil cancelButtonTitle:BTLocalizedString(@"确定") otherButtonTitles:nil, nil];
                [alert show];
                return;
            }
        };
